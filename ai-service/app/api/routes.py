...
async def chat_with_ai(
    ...
    # 生成会话ID（处理空字符串情况）
    session_id = request.sessionId if request.sessionId and request.sessionId.strip() else str(uuid.uuid4())
...
class AgentChatRequest(BaseModel):
    ...
    user_id: Optional[str] = "anonymous"
    question: str
    agent_type: Optional[str] = "auto"  # auto, nutrition, exercise, psychology, medical, health
    sessionId: Optional[str] = None
    enable_memory: Optional[bool] = True  # 是否启用记忆召回
    enable_knowledge: Optional[bool] = True  # 是否启用知识检索
    enable_voice: Optional[bool] = False  # 是否返回语音
...
class VoiceChatRequest(BaseModel):
    ...
    enable_knowledge: Optional[bool] = True


class DocumentGenerateRequest(BaseModel):
    """文档生成请求"""
    user_id: str
    doc_type: str  # health_report, diet_plan, exercise_plan
    title: str
    content: Dict[str, Any]
    output_format: Optional[str] = "pdf"  # pdf, word
...
async def agent_chat(
    ...
    - **agent_type**: 智能体类型 (auto/nutrition/exercise/psychology/medical/health)
    - **enable_memory**: 是否启用记忆召回
    - **enable_knowledge**: 是否启用知识检索
    - **enable_voice**: 是否同时返回语音合成
    """
    import asyncio
    import uuid
    import time
    ...
    logger.info(f"专家智能体对话 - 用户: {user_id}, 智能体: {request.agent_type}, 问题: {request.question[:50]}...")
    
    # 延迟加载智能体模块
    from app.agent import get_agent_manager, get_intent_classifier, Intent
    from app.memory import get_memory_retriever, format_memories
    from app.rag import get_retriever, format_retrieval_context
    
    agent_manager = get_agent_manager()
    intent_classifier = await get_intent_classifier()
    
    # 意图识别
    intent_result = await intent_classifier.classify_and_extract(request.question)
    logger.info(f"意图识别结果: {intent_result.intent.value}, 置信度: {intent_result.confidence:.2f}")
    
    # 如果agent_type为auto，根据意图自动路由
    agent_type = request.agent_type
    if agent_type == "auto":
        intent_to_agent = {
            Intent.DIET_PLAN: "nutrition",
            Intent.EXERCISE_PLAN: "exercise",
            Intent.MENTAL_SUPPORT: "psychology",
            Intent.REPORT_ANALYSIS: "medical",
            Intent.HEALTH_CONSULT: "health",
        }
        agent_type = intent_to_agent.get(intent_result.intent, "health")
        logger.info(f"自动路由到智能体: {agent_type}")
    ...
    if request.enable_memory:
        try:
            ...
            memories = await memory_retriever.retrieve_combined(user_id, request.question, top_k=5)
            if memories:
    ...
    # 知识检索
    knowledge_context = ""
    if request.enable_knowledge:
        try:
            retriever = get_retriever()
            knowledge_results = retriever.retrieve(request.question, top_k=3)
            if knowledge_results:
                knowledge_context = format_retrieval_context(knowledge_results)
                system_prompt += f"\n\n【相关知识】\n{knowledge_context}"
                logger.info(f"检索到 {len(knowledge_results)} 条知识")
        except Exception as e:
    ...
    async def producer():
        ...
        try:
            chunk_count = 0
            # 调用智能体对话（非流式）
            response = await agent.chat(messages, user_id=user_id, use_memory=request.enable_memory, use_rag=request.enable_knowledge)
            # 模拟流式输出，按句子分割
            import re
            ...
            logger.info(f"智能体响应完成，共 {chunk_count} 个片段")
        except Exception as e:
            logger.error(f"智能体响应错误: {e}")
            await queue.put(f"[ERROR]: {str(e)}")
            ...
            try:
                ...
                if request.enable_memory:
                    ...
                    await memory_store.add_memory(
                        user_id=user_id,
                        memory_type="fact",
                        content=f"用户询问: {request.question}, AI回答: {ai_response[:200]}...",
                        importance=5.0
                    )
                
                logger.info(f"对话历史已保存 - user_id: {user_id}, session_id: {session_id}")
            except Exception as e:
                logger.error(f"保存对话历史失败: {e}")
...
async def agent_voice_chat(
    file: UploadFile = File(...),
    agent_type: str = Query("auto", description="智能体类型"),
    enable_memory: bool = Query(True, description="是否启用记忆"),
    enable_knowledge: bool = Query(True, description="是否启用知识检索"),
    current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    语音对话接口
    
    接收语音文件，识别后交给智能体处理，返回文本和语音合成结果
    """
    import uuid
    import os
    import aiofiles
    ...
    logger.info(f"语音对话 - 用户: {user_id}, 智能体: {agent_type}")
    
    # 保存上传的语音文件
    temp_dir = "/tmp/voice_input"
    ...
    try:
        # 语音识别
        from app.multimodal import create_recognizer
        recognizer = create_recognizer()
        recognition_result = await recognizer.recognize_from_file(temp_file_path)
        
        if not recognition_result.success:
            raise HTTPException(status_code=400, detail=f"语音识别失败: {recognition_result.error_message}")
...
@router.post("/agent/document/generate")
async def generate_document(
    ...
    - **doc_type**: 文档类型 (health_report/diet_plan/exercise_plan)
    - **output_format**: 输出格式 (pdf/word)
    """
    try:
        user_id = current_user["user_id"]
        logger.info(f"生成文档 - 用户: {user_id}, 类型: {request.doc_type}, 格式: {request.output_format}")
        
        from app.multimodal import get_document_generator
        
        doc_generator = get_document_generator()
        
        if request.output_format == "pdf":
            doc_bytes, file_path = doc_generator.generate_pdf_document(
                title=request.title,
                content=request.content,
                template=request.doc_type
            )
        else:
            doc_bytes, file_path = doc_generator.generate_word_document(
                title=request.title,
                content=request.content,
                template=request.doc_type
            )
...
async def search_knowledge(request: KnowledgeSearchRequest):
    ...
    """
    try:
        logger.info(f"知识库搜索: {request.query}, 分类: {request.category}")
...
async def get_user_memories(
    ...
    - **memory_type**: 记忆类型过滤 (fact/preference/goal/summary)
    """
    try:
        # 只能查看自己的记忆
